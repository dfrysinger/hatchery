...
class TestAgentIsolationValidation:
    ...
    def test_isolation_group_non_string_handled_downstream(self):
        """Non-string 'isolationGroup' is handled by isolation parsing, not schema validation.
        
        The isolation parsing code coerces simple types (int → str) and
        falls back to sanitized agent name for complex types (dict, list).
        """
        habitat = {"name": "Test", "agents": [{"agent": "Claude", "isolationGroup": 123}]}
        rc, stdout, stderr = run_parse_habitat(habitat)
        # Should succeed — isolation parsing handles the coercion
        assert rc == 0
