...
- [x] Missing `sharedPaths` defaults to `[]` (or `["/clawd/shared"]` for `none` mode)
- [x] Missing `isolationGroup` uses agent name as implicit group
- [x] Missing `network` defaults to `"host"`
- [x] All tests in `TestBackwardCompatibility` pass (21 tests)
