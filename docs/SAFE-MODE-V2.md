...
>
> **Updated Feb 20 2026** to reflect the simplification plan implementation (see [SIMPLIFICATION-PLAN.md](SIMPLIFICATION-PLAN.md) for full details). Key changes: settle time 45→10s, TimeoutStartSec 420→180s, unified config generator (`generate-config.sh`), shared libraries (`lib-env.sh`, `lib-auth.sh`), E2E/intro separation, single-phase provisioning.
...
All JSON configs are generated by `generate-config.sh` using `jq` — no bash string interpolation for JSON values. Token validation lives in `lib-auth.sh`, auth header construction in `get_auth_header()`, notifications in `lib-notify.sh`. Each function exists in exactly one place.

### 8. Shared Libraries

Common code is extracted into libraries deployed to `/usr/local/sbin/`:
- `lib-env.sh` — base64 decode, env loading
- `lib-auth.sh` — token validation, auth headers, provider discovery
- `lib-notify.sh` — Telegram/Discord notifications, SafeModeBot intro
- `lib-health-check.sh` — health check utilities
- `lib-permissions.sh` — file/dir ownership
...
│                                                                  │
│  cloud-init runcmd → bootstrap.sh → provision.sh                 │
│  (Sequential, no background fork)                                │
│                                                                  │
│  Stage 1: Parse config, install Node/jq                          │
│  Stage 2: Install OpenClaw, create user                          │
│  Stage 3: Install desktop environment                            │
│  Stage 4: Install developer tools                                │
│  Stage 5: Install browser & pip packages                         │
│  Stage 6: Configure desktop & remote access                      │
│  Stage 7: Install skills & apps                                  │
│  Stage 8: Build configs & generate services                      │
│                                                                  │
...
│      → Do NOT reboot into broken state                           │
└──────────────────────────────┬───────────────────────────────────┘
                               │
                               ▼ REBOOT
                               │
...
`provision.sh` runs sequentially from cloud-init's `runcmd` context — no background fork, no race conditions. All package installs, config generation, and service creation happen in a single script with 9 progress stages. The script ends with a reboot.
...
**Purpose:** Verify agents can actually respond to messages. Runs after the gateway HTTP endpoint is confirmed alive. **E2E testing is separate from agent intros** — the health check uses a deterministic test prompt and never sends `--deliver`.

**Called by:** `openclaw-e2e-{group}.service` (separate systemd unit, `BindsTo` main service).

**Normal mode flow:**
1. `check_channel_connectivity()` — validate every agent's bot token via API (uses `validate_telegram_token()` / `validate_discord_token()` from `lib-auth.sh`)
2. `check_agents_e2e()` — prompt each agent with `"Reply with exactly: HEALTH_CHECK_OK"` using `--json` (no `--deliver`), verify it responds
3. All pass → mark healthy, then `send_agent_intros()` (separate, only on fresh boot — gated by `INTRO_SENT_MARKER`)
4. Any fail → write unhealthy marker (triggers safeguard .path unit)

**Safe mode flow (Run 2):**
1. `check_agents_e2e "safe-mode"` — same unified function, just testing the safe-mode agent
2. Pass → mark safe mode stable, `notify_send_safe_mode_intro()` from `lib-notify.sh`
...
9. `notify_send_safe_mode_intro()` — SafeModeBot introduces itself with AI-powered diagnostics (shared function from `lib-notify.sh`)
...
**Provides:**
- `send_telegram_notification(token, chat_id, message)` — low-level Telegram send
- `send_discord_notification(token, owner_id, message)` — low-level Discord send (auto-converts HTML to markdown)
...
- `notify_send_safe_mode_intro()` — generate boot report, send SafeModeBot intro via `openclaw agent --deliver` (shared by `gateway-e2e-check.sh` and `safe-mode-handler.sh`)

**Token validation:** Delegated to `lib-auth.sh` (`validate_telegram_token()`, `validate_discord_token()`). No local token validation functions — `lib-notify.sh` sources `lib-auth.sh` on load.
...
**Purpose:** Single source of truth for authentication — token validation, auth header construction, provider discovery, and token hunting. Extracted from `safe-mode-recovery.sh` to eliminate duplication.

**Provides:**
- `get_auth_header(provider, token)` — returns correct HTTP header (`x-api-key` vs `Authorization: Bearer` for Anthropic OAuth, etc.)
- `validate_telegram_token(token)` — call `getMe`, check response (with `TEST_MODE` support)
- `validate_discord_token(token)` — call `/users/@me`, check response (with `TEST_MODE` support)
- `validate_api_key(provider, key)` — provider-aware validation, sets `VALIDATION_REASON`; trusts OAuth tokens (`sk-ant-oat*`) without API call
- `find_working_telegram_token()` / `find_working_discord_token()` — search agents by group, set `FOUND_TOKEN_RESULT`
- `find_working_platform_token()` — try preferred platform, then cross-platform fallback
- `find_working_api_provider()` — try providers in fallback order (user's default → anthropic → openai → google)
- `get_provider_from_model(model)` — extract provider from model string (`anthropic/claude-opus-4-5` → `anthropic`)
- `get_provider_order()` / `get_default_model_for_provider(provider)` — provider utilities
...
**Provides:**
- `d(value)` — base64 decode helper (empty input → empty output)
- `env_load()` — source `/etc/droplet.env` and `/etc/habitat-parsed.env` (respects `TEST_MODE`)
- `env_decode_keys()` — decode common API keys from base64 env vars (won't overwrite pre-set values)

### `generate-config.sh` (~496 lines)

**Purpose:** Single source of truth for all OpenClaw JSON config generation. Uses `jq` exclusively — zero bash string interpolation for JSON values. Replaces 5 separate heredoc config generators.

**Modes:**
- `--mode full` — full production config (all agents, channels, env, browser, skills)
- `--mode session --group NAME --port PORT` — per-group session config (filtered agents)
- `--mode safe-mode --token TOKEN --provider PROVIDER` — safe mode recovery config
...
│
├─ Wait SETTLE seconds (10s default)
│
...
│   └─ YES → check_agents_e2e("safe-mode")  ← unified function
│            │ Prompt: "Reply with exactly: HEALTH_CHECK_OK" (--json, no --deliver)
│            ├─ Pass → mark stable, notify_send_safe_mode_intro() → exit 0
...
└─ Normal mode:
    ...
    │   │   validate_telegram_token() / validate_discord_token() (from lib-auth.sh)
...
│
├─ Service up? → notify_send_safe_mode_intro() from lib-notify.sh
│   (Generates BOOT_REPORT.md, SafeModeBot reads it, delivers AI diagnostics)
│
...
# Intro (separate, only on fresh boot — gated by INTRO_SENT_MARKER):
openclaw agent --agent "$agent_id" \
...
**Re-running health checks will NOT re-send intros** — the marker file prevents it.
...
```
lib-env.sh                    ← standalone
  ↑
lib-auth.sh                   ← sources lib-env.sh
  ↑
lib-notify.sh                 ← sources lib-auth.sh
  ↑
lib-health-check.sh           ← sources lib-env.sh
...
safe-mode-recovery.sh         ← sources lib-auth (via handler)
generate-config.sh            ← sources lib-env
```
