...
| `discord.ownerId` | If discord | Your Discord user ID for DM notifications |
| `telegram.ownerId` | If telegram | Your Telegram user ID for notifications |
| `agents` | **Yes (1+)** | At least one agent with bot token for notifications |
| `agents[].agent` | **Yes** | Agent name (becomes directory name) |
| `agents[].discordBotToken` | If discord | Bot token for Discord |
| `agents[].telegramBotToken` | If telegram | Bot token for Telegram |
...
```json
{
  ...
  "agents": [{ "agent": "bot", "discordBotToken": "MTIz..." }]
}
```
...
```json
{
  ...
  "agents": [{ "agent": "bot", "telegramBotToken": "123:ABC..." }]
}
```
