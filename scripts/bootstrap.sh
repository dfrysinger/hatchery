...
for f in "$INSTALL_DIR"/scripts/*.sh; do
  ...
  case "$bn" in
    ...
    phase1-critical.sh|phase2-background.sh|provision.sh|build-full-config.sh|generate-session-services.sh|generate-docker-compose.sh|install-docker.sh|lib-permissions.sh|lib-health-check.sh|lib-notify.sh|lib-env.sh|lib-auth.sh|lib-isolation.sh)
      cp "$f" /usr/local/sbin/
      ;;
    *)
      cp "$f" /usr/local/bin/
