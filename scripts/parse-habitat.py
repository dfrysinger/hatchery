...
def validate_agent_schema(agent, index):
    ...
    # Optional: isolationGroup (validated downstream by is_valid_isolation_group)
    # We allow non-string types here because the isolation parsing code
    # handles type coercion (int → str, etc.) with appropriate warnings.
    # Only reject None since that indicates a null value, not a type mismatch.
